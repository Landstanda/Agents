...
class TaskManager:
    ...
    async def execute_recipe(self, recipe: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            # Get urgency from NLP result (default to 5 if not specified)
            urgency = context.get("nlp_result", {}).get("urgency", 0.5)
            ...
            # Add to priority queue - use negative urgency for max-heap behavior
            # Also add a sequence number to maintain FIFO order for equal priorities
            async with self._task_lock:
                sequence = len(self._task_queue)
                heapq.heappush(self._task_queue, (-urgency, sequence, task))
            ...
            return {
                "status": "queued",
                "details": f"Task queued with urgency {urgency}",
                "task_id": task_id
            }
            
        except Exception as e:
            logger.error(f"Error queueing recipe: {str(e)}")
            return {
                "status": "error",
                "error": f"Failed to queue recipe: {str(e)}"
            }
    ...
    async def _process_queue(self):
        ...
        while True:
            async with self._task_lock:
                ...
                # Get highest priority task (ignoring sequence number)
            ...
            try:
                ...
                if task["recipe"]["name"] == "Error Handling":
                    ...
                    # Check for invalid intent before processing steps
                    if "invalid" in task["context"].get("nlp_result", {}).get("all_intents", []):
                        raise Exception("Invalid intent detected")
    ...
    async def _handle_api_call(self, step: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # This would integrate with various APIs based on the step configuration
        return {
            "status": "success",
            "details": f"API call to {step.get('endpoint', 'unknown')} completed"
        }
